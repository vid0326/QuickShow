    ...
    max-w-13 md:max-w-60 w-full border-r border-gray-300/20 text-sm'>
        ...
        <div className='w-full'>
            {adminNavlinks.map((link)=>(
                <NavLink key={link.path} to={link.path} end  className={({isActive})=>` group relative flex items-center 
                max-md:justify-center gap-2 w-full py-2.5 min-md:pl-10 first:mt-6
                text-gray-400 ${isActive && 'bg-[var(--color-primary)]/15  text-[var(--color-primary)] '}`}>
                ...
                {({isActive})=>{
                        ...
                        <>
                           ...
                           <span className={`w-1.5 h-10 rounded-1 right-0 absolute ${isActive && 'bg-[var(--color-primary)]/90' }`}></span>
