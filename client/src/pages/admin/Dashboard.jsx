...
const Dashboard = () => {
  ...
  const fetchDashboardData = async ()=>{
    ...
    setLoading(false)
      ...
      console.error('Error fetching dashboard data:', er)
      setLoading(true)
    }
  }
    ...
    <>
      ...
      <div className='relative flex flex-wrap gap-4 mt-6' >
        ...
        <div className='flex flex-wrap gap-4 w-full'>
          {dashbaordCards.map((card,index)=>(
            <div key ={index} className='flex items-center justify-between px-4 py-3 bg-[var(--color-primary)]/10 border border-[var(--color-primary)]/20 
            rounded-md max-w-50 w-full active:scale-95 transition-all duration-200 hover:bg-[var(--color-primary)]/20'>
              <div>
                <h1 className='text-sm'>{card.title}</h1>
                <h1 className='text-xl font-medium mt-1'>{card.value}</h1>
              </div>
